...
```
# Introduction
Extensive off-highway vehicle (OHV) activies can degrade fish habitat.
...
measureable change to the density of immature and adult fish, particularly bull trout.
The density of fish will be measurved using electro-fishing methods as
described in xxxxxx(xxxx) and the catch per unit effort (CUE) 
...
examine if change has occured in the mean CUE. Because restoration activities are on going,
there is little/no data collected post-restoration. Consequently, simulated data will be used
...
Fortunately, because the same stream is repeated measured over time, the stream
serves as its own control and so the stream-to-stream variation has litte impact on the
...
effects as needed. See the individual analyseses below for more details.

The $\mathit{lmerTest}$ package (Kuznetsova et al., 2018) in the $R$ software system (R Core Team, 2018).
...
to estimate, for example, pair-wise comparisons such a before vs. after, or baci effects without relying on 
...
* Each of the random effects (residual, year effects and site-effects) have a normal distribution
...
residual and caterpillar plots. A custom function (*sf.autoplot.lmer*), suitable for mixed linear models and 
patterend after similar plots in the
*ggfortify* package for linear models, is avaiable at:
[http://www.stat.sfu.ca/~cschwarz/Stat-650/Notes/MyPrograms/schwarz.functions.r]
and a copy is included in the directory with this report.
...
The two plots in the top row examine the residual, the difference between the observed and fitted data.
...
probabiity plot. If the residuals follow a normal distribution, the points should lie close to the
...
Outliers should be identied because they have the potential to influence the estimates when 
...
by year-specific effects (process error) are the limiting factor that determines the
...
Should such outlier random effects be removed, in this case, by dropping a year of data. There
is no simple answer to this -- presumably if the year-specific effect is caused by a 1-in-a-thousand-flood,
there are good reasons to drop this year. 

Similarly, random effects for streams (not shown) would be interpretted in the same way. Again, because
all streams are measured before and after the restoration action, stream effect again "cancel" in the BACI analysis 
...
We simulated six dataset in a before/after scenario -- similar findings are expected for the other models in
...
distribution for the residuals that is symmetic but has a much wider spread then a normal distribution.

A plot of the simulated data is:
...
The outlier in the first year is evident in the two plots; the impact of non-normality is not obvious from this plot
...
We fit the before/after model (see below) to the simulated data and extracted the residual plots
...
The presence of the outlier is evident in both plots regardless of sample size. The non-normality shows
it self only in the normal probability plots.
...
in the estimate because the outlier occured in the *before* period and so increased the estimates
of the mean in the *before* period relative to the *after* period. The bias decreases with increasing
sample size per year as then the rest of the data swamps this outlier.
...
We will again illustrate the impacts using simulated data - in this case a BACI design with 2 streams in each of 
impact and control and 4 sites measured per stream-year.
...
The outlier is apparent in all the plots except for that of stream. With only a few streams it is difficult to detect
...
Only the outlier in the stream-year random effects has any influence on the estimates or p-values for the
...
some sites are revisited, this approach is slightly inefficient (i.e. reported standare errors
...
over years (e.g. based on lat/long of the site location). This addition to the model is appropriate if all sites
...
(indicating no effect) but such a result is qualitatively different from another interval
$(-0.1 \rightarrow 1.9)$ that "barely" contains the value of 0. 

A Bayesian posterior belief of an effect has a very intuitive interpretation and
is easily computed once the mixed linear models are fit. Gelman and Hill (2006) show
that the estimates from a mixed model fit are also the modes of the posterior distribution
for that parameter with suitably chosen priors. Gelman and Su (2019) implement these ideas
...
positive which has a natural interpretation.


# Characteristics of the simulated data
The data is used to illustrate the analyses below is a combination of current data and simulated data.
All of the current data is treated as "Before" restoration data and streams are divided into their control/impact (restored)
status.

The simulated data starts in 2018 and ends in 2023. The 2018 year is treated as "Before" year so that 
all streams have at least two years of "Before" data (this is needed to fit the broken-stick model below).
[When the actual data are available, the start of the restoration period can be set separately for each watershed.]
...
No effect of restoration is simulated, and so the means before and after restoration should be similar.
[An effect could be simulated, but has no impact on the analysis below, other than to show an effect.]
...
an Excel worksbook or a csv file. We currently use the simulated data as described above.
...
detected. In many cases, database systems only record postive counts. A naive extraction of the data from such systems would 
not capture the 0 counts for species looked for but not seen.

This could lead to biases in estimates. For example, consider very simple example where:
...
A incorrect estimate of mean density over the
...
that is specfic to the visit, such as the length of the segment, the personell used, etc.
...
The CPUE is then computed as the $count / distance~sampled * 300$ to standardize per 300 m of effort for each visit.
...
The Oracle query in FWMIS is now designed to provide information on sites with zero fish encountered to aleviate
...
$log(\mathit{CUE}+\mathit{offset})$ where $log()$ refers to the natural
logarithmic transform and the $\mathit{offset}$ is 1/2 of the smallest non-zero value. 
...
i.e. different number of sites can be measured in for each stream-year 
...
The simplest analysis is a before-after analysis where the mean CPUE is compared betweeh the before restoration period
...
* Average the $log(CUE+offset)$ for each site-year combination and analyze the averages using
...
The model is
$$log(\mathit{CUE}+\mathit{offset}) \sim \mathit{BA} + \mathit{YearF}(R)$$
...
The fitted values and observed data can be plotted together. The blue line below is the fitted trend line
and the red line joins the mean of the log(CUE).
```{r  }
...
by adding the *(1|Site)* term to the model. The rest of the code can then be run without anyother
changes.
```{r}
# Get all of the fits and store them in a list with the emmeans computatons
...
ggplot(data=ba.diffs.rep, aes(y=paste(Watershed,".",Stream,sep=" "), x=estimate))+
   ggtitle("Estimated difference (log scale) between before and after",
           subtitle="Repeated measurements on the same site accounte for")+
   geom_point()+
   ggplot2::geom_errorbarh( aes(xmin=lower.CL, xmax=upper.CL), height=.01)+
   xlab("Estimated difference (log-scale) and 95% ci")+
   ylab("Watershed - Stream")+
...
at the point of restoration. For example, here is sample plot illusrating the line we wish to fit:
```{r echo=FALSE}
...
$$log(\mathit{CUE}+\mathit{offset}) \sim Year + YearP + \mathit{YearF}(R)$$
...
`data$YearP = pmax(data$Year - 2015)`.
...
bs.fits <- plyr::dlply(cue.data, c("Watershed","Stream","Species","AgeClass"), function(x){
   ...
   ci <- confint(fit2, "Year", quiet=TRUE)
...
We can examine the plots of fitted and predicted values to check te general form of the fit and
...
plyr::l_ply(1:npages, function(page){
  plot.fit <- ggplot(data=cue.data, aes(x=Year, y=logCUE))+
     ggtitle("Observed and fitted data from brocken stick model")+
     geom_point(position=position_jitter(width=0.1))+
...
bs.diffs.post <- plyr::ldply(bs.fits, function(x){
   ...
   post.samples <- arm::sim(x$fit, n.sims=n.sims.post)
   data.frame(post.diff.slope = -post.samples@fixef[,"YearP"]) # notice that R computes before-after
})
head(bs.diffs.post)
...
# BACI analysis within each watershed
## Introduction
...
but there must replication in at least one of the groups. If a watershed only has a single reference
...
$$log(\mathit{CUE}+\mathit{offset}) \sim \mathit{BA} + \mathit{CI} + \mathit{BA}:\mathit{CI} + $\mathit{YearF}$(R) + \mathit{Stream}(R)+ $\mathit{YearF}$:\mathit{Stream}(R)$$
...
and the $\mathit{YearF}:\mathit{Site}$ refers to the random noise where streams do not 
track each other in parallel over time.

The effects of stream will cancel in the analsis because each stream is measured
...
The last term represents inconsistency in the mean at a stream over time compared to other streams, i.e.,
...
## Using R to fit and extract results
We exclude the watersheds that don't have a control and treatment stream and a duplicate of either the control
...
Case where the 95% confidence interval does not include the value of 0 (vetical line) indicate
evidence of a BACI effect. The actual p-value is also shown on the plot

## Bayesian posterior beliefs
We again sample from the posterior distrbution of the parameters using the *arm* package. Some care
...
we notice what appears to be a "line" of residuals. This is an aftefact of the data because
it is not possible to have a CUE that is negative and and offset is added to all CUE values before
taking logarithms. For example, consider an observed value of CUE of 0 
where the offset was .0067 (2 fish/300 m). The observed value of $log(\mathit{CUE}+\mathit{offset})=-5$.
...
## Introduction
The impact of restortion may not be equal for all watersheds, i.e. variation may also exist in the BACI effect.
In this section, we will fit a super-model for all streams that estimates the averge effect of restoration
...
The statistical model for each watershed is
$$log(\mathit{CUE}+\mathit{offset}) \sim \mathit{BA} + \mathit{CI} + \mathit{BA}:\mathit{CI} + 
...
data file indictes which year belong to which period for each watershed.

We implicitly assume that year-specific effects apply to all watersheds. This can be 
assessed by adding an watershed-year interaction and looking at the size of the resulting
variance component.
...
Again, the "line" in the residuals is an artefact of CUE being limited below by 0. As noted previously,
the distribution of the residuals at the lowest level is not that crucial because measurements at
sites are essentially being averaged for each year. 
...
program. The analyzes become progressively more complex so that deficiencies of each 
 ...
 * The BACI design uses stream(s) that have not been restored as controls to account for temporal change.
 ...
 Only having a single treatment stream, unfortunately, still leave some question that the results
 ...
 * We did not examine a BACI analysis where a change in slopes was investigated. This model
 is straightforward and would simply involve including terms for the trend during the before and after periods 
 in the control and impact streams.
 
 * Finally, a global BACI analysis combined results over watersheds. Here the BACI effect is a 
 "global" average effect. Consequently, the the restoration actions are quite different across
 watersheds, this average BACI effect must be interpretted with care.
...
these statistics do not have an intutive interpretation. For example, many people will interpret a p-value of 0.01 as
...
the posterior distribution of a parameter can be interpretted as the "belief" in particular values of the 
parameters. So it is sensible to compute the posteriour belief that the effect is positive. Fortunately, 
