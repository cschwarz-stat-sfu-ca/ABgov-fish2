...
all species. A revised slope ($\beta_\textit{after}-\beta_\textit{before}$) was used to modify the CUE (on the log-scale) as shown below:
```{r echo=FALSE}
effect.size.slope <- read.csv("effects.size.slope.csv", header=TRUE, as.is=TRUE, strip.white=TRUE)
effect.size.slope
```

The new slope apply to all age classes (if there are more than one).
