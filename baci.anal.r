...
##############################################################################
##############################################################################
# Before/After analysis for Falls
...
scenario <- expand.grid(BA.eff= log(2), 
                     n.years.b=c(3,4,5),
                     n.years.a=c(1:5,7,10,15),
              n.sites.per.year=c(5,10,15,20),
                  alpha=c(0.05, 0.10, 0.15),
                                    stringsAsFactors=FALSE)
scenarios <- merge(vc2[vc2$Watershed=="Ram",], scenario)  # Only Ram watershed needs a Before/After Analysis.

vc2[vc2$Watershed=="Ran",]


ba.power <- plyr::adply(scenarios,1, function(x){
  power <- before.after.power.stroup(Shift=x$BA.eff, 
                                     X.before=rep(1:x$n.years.b, each=x$n.sites.per.year),
                                     X.after=rep(20:(20+x$n.years.a), each=x$n.sites.per.year), 
                                     Process.SD=x$sd.Year, Sampling.SD=x$sd.Resid, alpha=x$alpha)
...
pdf(file.path("Report",paste("Figure05-power-plot-BA.pdf")), h=6, w=6)
plyr::d_ply(ba.power, c("alpha","Watershed","Species","n.years.b"),function(x){
  cat("Plotting ",x$alpha[1], x$Watershed[1], x$Species[1], x$n.years.b[1], "\n")
  #if(x$alpha[1]>0.10)browser()
  power.plot <- ggplot(data=x, aes(x=n.years.a, y=os.power1,  color=as.factor(n.sites.per.year)))+
    ggtitle(paste("BA power to detect a ", exp(x$Shift[1]), 'x increase in mean CUE\n',
                  x$Watershed[1], " ",x$Species[1], " with ", x$n.years.b[1]," years measured before restoration takes place",
  ...
  plot(power.plot) 
#  ggsave(power.plot, 
#         file=file.path("Report",paste("power-plot-BA-",x$Watershed[1],"-",x$n.years.b[1],"-years-before-",
...
##############################################################################
##############################################################################
# Separate power for each watershed
...
power <- plyr::adply(scenarios, 1, function (x){
  ...
  power <-  baci.power( n_TA=x$n.sites.per.year,
                        n_TB=x$n.sites.per.year,
                        n_CA=x$n.sites.per.year,
                        n_CB=x$n.sites.per.year,
                        ns_T=x$n.streams.t,
                        ns_C=x$n.streams.c,
                        ny_B=x$n.years.b,
                        ny_A=x$n.years.a,
                        mu_TA=x$BACI.eff,       mu_TB=0,    mu_CA=0,   mu_CB=0,
                        sdYear=x$sd.Year, 
                        sdSite=x$sd.Stream, 
                        sdSiteYear=x$sd.StreamYear, 
                        sdResid=x$sd.Resid,          alpha=x$alpha)
  x <- cbind(x, power)  
  x
  
}, .parallel=doParallel)

write.csv(power, file.path("Report","power.csv"), row.names=FALSE)

n.controls <- plyr::ddply(power, c("Watershed","AgeClass"), plyr::summarize,
              n.streams.c=mean(n.streams.c))
n.controls
n.controls$n.streams.c2 <- paste(n.controls$n.streams.c,' controls',sep="")
n.controls

# graph the power curves
pdf(file.path("Report",paste("Figure06-power-plot-BACI.pdf")), h=6, w=6)
plyr::d_ply(power, c("BACI.eff","alpha","Species","n.years.b"),function(x){
   cat("Plotting ", x$BACI.eff[1], x$alpha[1], x$Species[1], x$n.years.b[1], "\n")
   power.plot <- ggplot(data=x, aes(x=n.years.a, y=os.power1,  color=as.factor(n.sites.per.year)))+
      ggtitle(paste("BACI power to detect a ", exp(x$baci[1]), 'x increase in mean CUE - ',x$Species[1],
